...
double nloglik_M0Z_glm = function(NumericVector y,NumericVector z,NumericVector x, NumericVectorcp) {            //y   : response
// z   : predictor without measurement error
// x   : predictor with varying coefficient
// cp  : changepoint chromosome (m; tau_1,___,tau_m)
// link: link function for Poisson regression
  ...
  if (m == 0) {
    ...
    x_spl = outer(x,cp[-1],">")*outer(x,cp[-1],"-");
    X = cbind(z,x,x_spl);
  }
  p = ncol(X)-m;

    fit_MLE_out = glm(y~X,family=gaussian(link="identity"),start=c(rep(0_5,1+p),rep(0,m)));
    nllik_glm = -as_numeric(logLik(fit_MLE_out));

    return nllik_glm;
}

////// Compute the penalized log-likelihood with MDL penalty
pnllik_MDL_M0Z = function(NumericVector y,NumericVector z,NumericVector x,NumericVector cp,
                          double x_min, double x_max, double x_inc) {             // y   : response
// z   : predictor without measurement error
// x   : predictor with varying coefficient
// i_g : indicator variable for group
// cp  : changepoint chromosome (m; tau_1,___,tau_m)
// link: link function for Poisson regression
  pnllik_MDL = nloglik_M0Z_glm(y=y,z=z,x=x,cp=cp)+penalty_MDL(y=y,x=x,cp=cp,x_min=x_min,x_max=x_max,x_inc=x_inc)
