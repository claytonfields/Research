...
penalty.MDL <- function(y,x,cp) {                  # y   : response
...
pnllik.MDL.M0 <- function(y,x,cp) {       # y   : response
                                                   # x   : predictor with varying coefficient
                                                   # i.g : indicator variable for group
                                                   # cp  : changepoint chromosome (m; tau_1,...,tau_m)
                                                   # link: link function for Poisson regression
  pnllik.MDL <- nloglik.M0_glm(y=y,x=x,cp=cp)+penalty.MDL(y=y,x=x,cp=cp)
...
### Compute the penalized log-likelihood with MDL penalty
pnllik.MDL.M0Z <- function(y,z,x,cp) {             # y   : response
                                                   # z   : predictor without measurement error
                                                   # x   : predictor with varying coefficient
                                                   # i.g : indicator variable for group
                                                   # cp  : changepoint chromosome (m; tau_1,...,tau_m)
                                                   # link: link function for Poisson regression
  pnllik.MDL <- nloglik.M0Z_glm(y=y,z=z,x=x,cp=cp)+penalty.MDL(y=y,x=x,cp=cp)
